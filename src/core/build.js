...
module.exports = async (cliConfig, custumConfig) => {
  const pkg = pkgLoader()
  const packConfig = configLoader(process.cwd(), pkg, cliConfig, custumConfig)

  if (!packConfig.skipAlert) {
    const checkOutputDir = async () => {
      if (existsSync(packConfig.output.directory)) {
        console.log('\n')
        const answers = await require('inquirer').prompt([
          {
            ...
            message: `Directory already exists, remove the dir: ${path.join(
              path.relative(process.cwd(), packConfig.output.directory)
            )}`
          }
        ])
        if (answers?.remove) {
          rimraf.sync(packConfig.output.directory)
        } else {
  ...
  if (!fs.existsSync(packConfig.output.directory)) {
    fs.mkdirSync(packConfig.output.directory)
  }
